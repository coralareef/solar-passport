...
def irr(cashflows: list[float]) -> Optional[float]:
    if not cashflows or not any(x < 0 for x in cashflows) or not any(x > 0 for x in cashflows):
        return None
    def fn(r: float) -> float: return npv(r, cashflows)
    lo, hi = -0.95, 1.0
    flo, fhi = fn(lo), fn(hi)
    for _ in range(16):
        if flo * fhi <= 0: break
...
def annuity_payment(principal: float, annual_rate: float, years: int) -> float:
    ...
    if abs(annual_rate) < 1e-12: return principal / years
...
def _validate(i: ProjectFinanceInputs) -> None:
    if i.capex_bnd <= 0: raise ValueError("capex_bnd must be > 0")
    if i.year1_p50_mwh <= 0: raise ValueError("year1_p50_mwh must be > 0")
    if not 0 < i.p90_factor <= 1: raise ValueError("p90_factor must be in (0, 1]")
    if i.ppa_bnd_per_kwh < 0: raise ValueError("ppa_bnd_per_kwh must be >= 0")
    if i.ppa_years < 1: raise ValueError("ppa_years must be >= 1")
    if not 0 <= i.debt_pct <= 1: raise ValueError("debt_pct must be between 0 and 1")
    if i.debt_tenor_years < 1 or i.debt_tenor_years > i.ppa_years: raise ValueError("debt tenor must be within PPA term")
...
def _cfads_series(i: ProjectFinanceInputs, p90: bool) -> list[float]:
    factor = i.p90_factor if p90 else 1.0
    out = []
    for year in range(1, i.ppa_years + 1):
...
def model_project_finance(i: ProjectFinanceInputs) -> ProjectFinanceResult:
    ...
    if i.dsra_funded_by_equity and dsra > 0:
        equity_flows[i.debt_tenor_years] += dsra
    ...
    return ProjectFinanceResult(irr(project_flows), irr(equity_flows), npv(i.discount_rate, project_flows), discounted_cost / discounted_energy_kwh if discounted_energy_kwh > 0 else None, debt, equity, annual_ds, dsra, tuple(p50_dscr), tuple(p90_dscr), min_p90, _llcr(p50, debt, i.debt_rate, i.debt_tenor_years), _llcr(p90, debt, i.debt_rate, i.debt_tenor_years), tuple(p50), tuple(p90), tuple(project_flows), tuple(equity_flows))
...
def solve_tariff(i: ProjectFinanceInputs, objective: str, *, lower: float = 0.0, upper: float = 1.0) -> Optional[float]:
    def passes(t: float) -> bool:
        ...
        if objective == "equity_irr": return result.equity_irr is not None and result.equity_irr >= i.target_equity_irr
        if objective == "p90_dscr": return result.minimum_p90_dscr is None or result.minimum_p90_dscr >= i.minimum_p90_dscr
        if objective == "npv": return result.project_npv_bnd >= 0
...
def debt_capacity_for_dscr(i: ProjectFinanceInputs, *, max_debt_pct: Optional[float] = None) -> float:
    ...
    if max_debt <= 0: return 0.0
    p90 = _cfads_series(i, True)
    min_cfads = min(p90[:i.debt_tenor_years])
    max_service = min_cfads / i.minimum_p90_dscr
    if i.debt_rate == 0:
        dscr_debt = max_service * i.debt_tenor_years
    else:
        dscr_debt = max_service * (1 - (1 + i.debt_rate) ** (-i.debt_tenor_years)) / i.debt_rate
